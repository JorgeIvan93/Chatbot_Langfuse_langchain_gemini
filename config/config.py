...
from pydantic_settings import BaseSettings, SettingsConfigDict  # Settings loader backed by .env/env

ConsoleLevel = Literal["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"]  # Allowed console levels
...
class Settings(BaseSettings):
    ...
    #  LLM (Google Gemini) 
    gemini_api_key: str = Field("", alias="GOOGLE_API_KEY")  # API key for Gemini Developer API
    gemini_model: str = Field("gemini-2.5-pro", alias="GEMINI_MODEL")  # Default chat model
    LLM_TEMPERATURE: float = Field(0.7, alias="LLM_TEMPERATURE")  # Default decoding temperature

    #  Langfuse (Observability) 
    LANGFUSE_PUBLIC_KEY: str = Field("", alias="LANGFUSE_PUBLIC_KEY")  # Public key for Langfuse
    LANGFUSE_SECRET_KEY: str = Field("", alias="LANGFUSE_SECRET_KEY")  # Secret key for Langfuse
    LANGFUSE_BASE_URL: str = Field("", alias="LANGFUSE_BASE_URL")  # Preferred base URL (region-aware)
    LANGFUSE_HOST: str = Field("https://cloud.langfuse.com", alias="LANGFUSE_HOST")  # Fallback host
    ...
    #  App 
    APP_NAME: str = Field("Advanced LangGraph Chatbot", alias="APP_NAME")  # Banner title
